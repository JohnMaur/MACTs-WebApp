...
const GymReportPDF = ({ data, selectedDate }) => {
  ...
  const tableData = data.map((item) => [
    item.gym_tupID,
    `${item.gym_firstname} ${item.gym_middlename} ${item.gym_lastname}`,
    item.gym_course,
    item.gym_section,
    item.gym_InHistoryDate,
    item.gym_OutHistoryDate,
  ]);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.settingSubject}>
          GYM TIME IN/OUT DAILY RECORD - {new Date(selectedDate).toLocaleDateString()}
        </Text>
