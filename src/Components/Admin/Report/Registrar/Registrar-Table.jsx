...
const RegistrarTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try { 
        const response = await axios.get('http://localhost:2526/Registrar-Report');
        const responseData = response.data;
        const transformedData = responseData.map((item, index) => ({
          key: `${index + 1}`,
          tuptId: item.registrar_tupID,
          name: `${item.registrar_firstname} ${item.registrar_middlename} ${item.registrar_lastname}`,
          email: item.registrar_email,
          course: item.registrar_course,
          section: item.registrar_section,
          date: item.registrar_taphistoryDate,
        }));
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        <p className="text-xl font-bold mb-2">Registrar Report</p>
