...
const LibraryTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try { 
        const response = await axios.get('https://macts-backend-webapp.onrender.com/Library-Report');
        const responseData = response.data;
        const transformedData = responseData.map((item, index) => ({
          key: `${index + 1}`,
          tuptId: item.library_tupId,
          name: `${item.library_firstName} ${item.library_middleName} ${item.library_lastName}`,
          course: item.library_course,
          section: item.library_section,
          TimeIN: item.library_InHistoryDate,
          TimeOut: item.library_OutHistoryDate,
        }));
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        <p className="text-xl font-bold mb-2">Library Report</p>
