...
const GymTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try { 
        const response = await axios.get('http://localhost:2526/Gym-Report');
        const responseData = response.data;
        const transformedData = responseData.map((item, index) => ({
          key: `${index + 1}`,
          tuptId: item.gym_tupID,
          name: `${item.gym_firstname} ${item.gym_middlename} ${item.gym_lastname}`,
          course: item.gym_course,
          section: item.gym_section,
          TimeIN: item.gym_InHistoryDate,
          TimeOut: item.gym_OutHistoryDate,
        }));
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        <p className="text-xl font-bold mb-2">Gym Report</p>
